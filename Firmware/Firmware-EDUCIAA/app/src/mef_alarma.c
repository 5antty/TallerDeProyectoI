...
static void pedido_entrada_PIN(code_action_t action, uint32_t timeout_ms)
{
    ...
    if (action == CODE_ACTION_ARM)
    { // ambos casos se manejan en el MEF_UPDATE() donde se pregunta si hay entrada_pin_activo = true;
...
static void pin_ingresado_correcto(void)
{ // PIN correcto
    if (pin_funcion == CODE_ACTION_ARM)
    ...
    else if (pin_funcion == CODE_ACTION_DISARM)
    { // QUIERO DESARMAR LA ALARMA-> apagar sirena y volver a reposo
...
static void pin_ingresado_fallo(void)
{ // PIN incorrecto
...
{ // paso PIN por parametro, importante para actualizar
    if (pin == NULL)
        return;
...
void MEF_Alarm_Update(void)
{
    ...
    sensor_pir_activo = (gpioRead(PIN_PIR) == HIGH);
    sensor_magnetico_activo = (gpioRead(PIN_MAGNETICO) == LOW);
    ...
    if (estado != estado_anterior)
    {
        ...
        txCmd('A', estado_a_string(estado)); // Enviar estado a pantalla (comando 'A' por ejemplo)
    ...
    case ALM_ARMANDO:
        ...
        if (TimerService_Elapsed(t_gracia - ARM_DELAY_GRACIA_MS, ARM_DELAY_GRACIA_MS))
        { // check t_sistema >= t_gracia
    ...
    case ALM_ARMADO:
    {
        ...
        if (sensores_activados == SENSOR_MAGNETICO || sensores_activados == SENSOR_AMBOS)
        {
            if (sensor_magnetico_activo == TRUE)
            { // Usamos la variable ya evaluada arriba
                uartWriteString(UART_USB, "DISPARO: �Puerta ABIERTA detectada!\r\n");
                intruso = true;
            }
        }

        // 2. REVISION PIR
        if (sensores_activados == SENSOR_MOVIMIENTO || sensores_activados == SENSOR_AMBOS)
        {
            if (sensor_pir_activo == TRUE)
            { // Usamos la variable ya evaluada arriba
                uartWriteString(UART_USB, "DISPARO: �Movimiento PIR detectado!\r\n");
                intruso = true;
            }
        }
    ...
    case ALM_IDENTIFICACION:
        ...
        if (TimerService_Elapsed(t_estado, INGRESANDO_PIN_DELAY))
        { // veo si se venci� INGRESANDO_PIN_DELAY
            estado = ALM_ACTIVADO;
            t_estado = t_sistema;
