...
void RIT_IRQHandler(void)
{
    ...
    if (timer_tick_counter > (SEMICICLO_TICKS + 20))
        zcd_flag = false;
}
...
{
    if (brillo >= 95)
        return 5; // M�nimo retardo seguro (500us)
    if (brillo <= 5)
        return SEMICICLO_TICKS + 50;
    return 100 - brillo;
}
...
void MEF_Luz_Update(void)
{
    ...
    for (int i = 0; i < MAX_FOCOS; i++)
    {
        ...
        case LUZ_ON:
            f->delay_disparo = 20; // 1.5ms para asegurar disparo

            div_debug++;
            if (div_debug > 20)
            { // Ajusta esto si sale muy r�pido
...
void MEF_Luz_SetMode(uint8_t id, luz_estado_t modo, uint8_t brillo)
{
    ...
    if (modo == LUZ_ON)
    {
        ...
        focos[id].brillo = 80; // Solo para guardar el dato
...
void MEF_Luz_SetRTC_OnTime(uint8_t id, uint8_t hora, uint8_t min)
{
    if (id >= MAX_FOCOS)
        return;
    focos[id].rtc_on_h = hora;
    focos[id].rtc_on_m = min;
}
...
void MEF_Luz_SetRTC_OffTime(uint8_t id, uint8_t hora, uint8_t min)
{
    if (id >= MAX_FOCOS)
        return;
    focos[id].rtc_off_h = hora;
    focos[id].rtc_off_m = min;
}